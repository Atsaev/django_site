...
Личный сайт-портфолио на Django: витрина проектов, блог, резюме с генерацией PDF и карточки для тренировки (флешкарты). Сделан как демонстрационный проект для поиска работы Python-разработчиком — с продакшн-деплоем через CI/CD (GitHub Actions → GHCR), а не просто локальным тудулистом.
...
- **Таймлайны рубрик** — хронологический вид любой категории: `/blog/timeline/<slug>/` (напр. `/blog/path/` — короткий URL рубрики «Путь» со статистикой откликов/собеседований/офферов)
- **Челленджи** — факультативные спринты «День N из M» с сеткой дней (выполнен/пропущен/сегодня/впереди), прогресс по факту написанных постов
...
- **Флешкарты** — тренировка «вопрос → ответ» с переворотом карточки; выбор одной/нескольких/всех тем (`/flashcards/`), контент редактируется в админке
...
├── flashcards/          # флешкарты: Topic, Card + тренировка
...
## Таймлайн-рубрики и челленджи

### Таймлайн рубрики

У любой категории блога есть хронологический вид — **таймлайн** (`/blog/timeline/<slug>/`), где посты идут от раннейших к новым. Специальный короткий URL `/blog/path/` ведёт на таймлайн рубрики «Путь». Никакой отдельной настройки не нужно: таймлайн создаётся автоматически при создании категории.

- **Статистика поиска работы** (отклики/собеседования/офферы) и статус профиля показываются только на таймлайне рубрики «Путь» (`/blog/path/`).
- Числа считаются суммой счётчиков по всем постам (`applications_count`/`interviews_count`/`offers_count`), бейджи у каждого поста — по его полям.

### Челленджи

Челлендж — факультативный спринт «День N из M» (например, «30 дней после курса»). Настраивается в админке `/admin/blog/challenge/`:

- **Дата старта** и **длительность** (число дней) задаются при создании.
- **День поста** (`day_number`) вводить не нужно — он вычисляется автоматически от **даты создания поста** и даты старта челленджа. Так пропуск дня честно отображается как разрыв в сетке (день без поста помечается «пропущен»).
- **Сетка дней** на странице челленджа и на главной показывает каждый день: выполнен / пропущен / сегодня / впереди.
- Вердикт: челлендж завершён 🎉, когда закрыты все дни (в т.ч. досрочно), либо провален 💔, если период закончился с пропусками.
- Нельзя уменьшить `total_days` ниже уже написанных дней — защита от выпадения постов из сетки.

## Флешкарты

Публичная тренировка «вопрос → ответ» с эффектом переворота карточки — без логина и без сохранения прогресса на сервер.

- **URL:** `/flashcards/` (ссылка в меню: `GET /flashcards`).
- **Выбор:** отмечаешь одну, несколько или все темы → «Начать тренировку».
- **Механика:** клик по карточке (или Space/Enter) переворачивает её и показывает ответ, кнопка «Следующий вопрос →» листает дальше. Карточки идут в случайном порядке.
- **Контент:** модели `flashcards.Topic` и `flashcards.Card` редактируются в админке `/admin/flashcards/` (темы с инлайн-карточками).

Стартовые наборы по темам можно загрузить командой:

```bash
uv run python manage.py seed_flashcards
```

Команда идемпотентна — повторный запуск не создаёт дубликаты (темы пересоздаются вместе с карточками).
...
> **Статика:** `collectstatic` выполняется на этапе сборки образа, поэтому том для `staticfiles` не нужен и насколько не монтируется — иначе после обновлений осталась бы старая статика. В медиа-контейнера монтируется только общий с Caddy том `media_volume` (фактическое имя — `portfolio_media_volume`, см. ниже).
...
| `seed_flashcards` | Заполняет флешкарты стартовыми наборами по темам (Python, Django, FastAPI, ООП, SQL, Docker, Git) |

Все команды идемпотентны — можно запускать повторно, старые записи заменяются новыми.
